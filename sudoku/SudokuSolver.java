...
public class SudokuSolver {
	...
	//set grid here
	static int[][] grid = grid9x9;

	static int subGrid = (int)Math.sqrt(grid.length);
	...
	public static boolean possible(int y, int x, int n) {
		...
		int x0 = (x/subGrid)*subGrid;
		int y0 = (y/subGrid)*subGrid;
	...
	public static boolean solve(int xJmp, int yJmp) {
		for (int y = yJmp; y < grid.length; y++) {
			for (int x = (y!=yJmp)?0:xJmp; x < grid[0].length; x++) {
				if(grid[y][x]==0) {
	...
	public static void printArr(){
		...
		for (int i = 0; i < grid.length; i++) {
			for (int j = 0; j < grid[0].length; j++) {
				System.out.print(((grid[i][j]>9)?grid[i][j]:" "+grid[i][j])+", ");
			}
			System.out.println();
		}
	}
